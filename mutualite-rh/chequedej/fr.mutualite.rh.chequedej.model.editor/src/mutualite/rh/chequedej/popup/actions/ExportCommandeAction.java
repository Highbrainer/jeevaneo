...
public class ExportCommandeAction implements IObjectActionDelegate {
	...
	protected void exportCommandes(File file, IProgressMonitor monitor) {
		...
		codesClient.forEach(codeClient -> {
			...
			try {
				System.out.println("Code Client " + codeClient.getLibelle() + "(" + codeClient.getNumero() + ")");
	...
	private void exportCommande(CodeClient codeClient, File dir, SubMonitor newChild) throws IOException {
		...
		URL url = bundle.getEntry("/template/template-commande2.xls");
		Workbook wb = new HSSFWorkbook(url.openStream());
		fillIn(codeClient, wb);

		try (FileOutputStream fileOutputStream = new FileOutputStream(new File(dir,
				"Commande ChequeDej " + DateFormat.getDateInstance().format(commande.getDate()) + " - " + codeClient.getLibelle() + " " + codeClient.getNumero() + ".xls"));) {
			wb.write(fileOutputStream);
			wb.close();
		}

		Workbook wb2 = new XSSFWorkbook();
		fillIn(codeClient, wb2);

		try (FileOutputStream fileOutputStream = new FileOutputStream(new File(dir, "Données de Commande ChequeDej " + DateFormat.getDateInstance().format(commande.getDate()) + " - "
				+ codeClient.getLibelle() + " " + codeClient.getNumero() + ".xlsx"));) {
			wb2.write(fileOutputStream);
			wb2.close();
		}
	}

	public void fillIn(CodeClient codeClient, Workbook wb) {
		Sheet sheet = getOrCreateSheet(wb, "Commande");
		log.info("Code Client : " + codeClient.getLibelle() + " (" + codeClient.getNumero() + ")");
		...
		log.info("Trouvé " + pairs.size() + " items pour le codeClient " + codeClient.getLibelle() + " " + codeClient.getNumero());

		for (int i = 0; i < pairs.size(); ++i) {
			// On commence à la ligne 15
			Item item = pairs.get(i).get1();
			Employe employe = pairs.get(i).get2();
			log.debug("Remplissage ligne " + (14+i) + " avec la commande pour " + employe.getLabel());
		...
		for (int i = 14+pairs.size(); i < sheet.getLastRowNum()+1; ++i) {
			Row line = getOrCreateRow(sheet, i);
			for(int n=0;n<8;++n) {
				log.debug("Effaçage de la ligne " + i + " cellule " + n);
				getOrCreateCell(line, n).setCellType(CellType.BLANK);
			}
		}
