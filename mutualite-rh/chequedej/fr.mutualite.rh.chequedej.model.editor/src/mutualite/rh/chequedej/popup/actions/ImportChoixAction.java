...
public class ImportChoixAction implements IObjectActionDelegate {
	...
	public void run(Shell aShell) {
		...
		ChangeCommand command = new ChangeCommand(root) {
			...
			public String getLabel() {
				return "Import des choix individuels";
			}
			...
			protected void doExecute() {
				...
				try {
					Files.lines(in.toPath(), StandardCharsets.ISO_8859_1).skip(1).map(line -> line.split("\\s*;\\s*")).forEach(a -> {
						int matricule = Integer.parseInt(a[0]);
						int nbCheques = Integer.parseInt(a[3]);
						root.getOrCreateChoix(matricule).setNbMaxCheques(nbCheques < 0 ? null : nbCheques);
					});
					ChequedejItemProviderAdapterFactory adapterFactory = new ChequedejItemProviderAdapterFactory();
					IItemLabelProvider adapter = (IItemLabelProvider) adapterFactory.createChoixIndividuelAdapter();
					
					ECollections.sort(root.getChoixIndividuels(), (ci1, ci2) -> adapter.getText(ci1).compareTo(adapter.getText(ci2)));
					} catch (Throwable e) {
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if (selection instanceof IStructuredSelection) {
			...
			if (element instanceof ChequeDej) {
				root = ((ChequeDej) element).getChoix();
